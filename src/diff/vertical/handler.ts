...
export class VerticalDiffHandler implements vscode.Disposable {
  ...
  private async insertDeletionBuffer() {
    ...
    this.redDecorationManager.addLines(this.currentLineIndex - this.insertedInCurrentBlock, this.deletionBuffer.length);
  ...
  async acceptRejectBlock(accept: boolean, startLine: number, numGreen: number, numRed: number) {
    if (numGreen > 0) {
