...
import { VerticalDiffHandler, type VerticalDiffHandlerOptions } from './handler'
...
export class VerticalDiffManager {
  ...
  async streamDiffLines(
    ...
    const diffHandler = this.createVerticalDiffHandler(
      ...
      {
        instant,
        onStatusUpdate: (_status, _numDiffs, _fileContent) => {}
  ...
  async acceptRejectAllChanges(accept: boolean, fileUri?: string) {
    ...
    const document = await vscode.workspace.openTextDocument(
      decodeUriAndRemoveFilePrefix(fileUri)
    )
    await document.save()
  }
}
