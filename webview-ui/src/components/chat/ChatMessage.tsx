...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, handleFeedback }) => {
  ...
  const renderContent = () => {
    if (message.content.type === MessageType.Text || message.content.type === MessageType.TextDelta) {
      return (
        <MarkdownRender
          role={message.role}
          mdString={message.content.type === MessageType.Text ? message.content.text : message.content.value}
        />
      );
    }
