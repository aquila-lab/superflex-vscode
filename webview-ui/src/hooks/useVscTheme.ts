...
function constructTheme(tmTheme: Record<string, any>): Record<string, string> {
  const rules = tmTheme.rules || [];

  const tokenToForeground: Record<string, string> = {};
  rules.forEach(({ token, foreground }: { token: string; foreground: string }) => {
    if (!foreground || !token) {
      return;
    }
  ...
  return Object.keys(theme).length === 0 ? fallbackTheme() : theme;
}
...
export function useVscTheme() {
  const [theme, setTheme] = useState<Record<string, string>>(constructTheme({}));
